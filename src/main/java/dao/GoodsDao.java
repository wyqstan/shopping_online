...
public class GoodsDao {
	...
	public List<Map<String, Object>> getHotGoodsList() throws SQLException {
		QueryRunner r =new QueryRunner(DBUtil.getDataSource());
		String sql="select g.id,g.name,g.cover,g.price  from goods g  order by sales desc limit 9";
		return r.query(sql, new MapListHandler());
	}
	public Map<String, Object> getScrollGoods() throws SQLException {
		QueryRunner r =new QueryRunner(DBUtil.getDataSource());
		
		String sql="select g.id,g.name,g.cover,g.price  from goods g  order by sales desc limit 1";
	...
	public int getGoodsCount(int typeId) throws SQLException {
		...
		if(typeId==0) {
			...
			int tmp= r.query(sql, new ScalarHandler<Long>(),typeId).intValue();
			return tmp;
		}
	}
	...
	public List<Goods> selectGoodsRecommend(int type,int pageNo,int pageSize) throws SQLException {
		...
		String sql="select g.id,g.name,g.cover,g.image1,g.image2,g.intro,g.price,g.stock,t.name typename from goods g ,recommend r,type t where g.id=r.goods_id and r.type=? and g.type_id=t.id limit ?,?";
	...
	public int selectGoodsSales(int id)throws SQLException{
		QueryRunner r=new QueryRunner(DBUtil.getDataSource());
		String sql="select sales from goods where id=?";
	...
	public void updatesales(int id,int sales) throws SQLException{
		QueryRunner r=new QueryRunner(DBUtil.getDataSource());
		String sql="update goods set sales=? where id=?";
		r.update(sql,sales,id);
	}
